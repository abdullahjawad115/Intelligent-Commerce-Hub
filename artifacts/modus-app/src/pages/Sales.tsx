      ...
      <div style={{ flex:1, overflowY:'auto', padding:'28px 32px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:22 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button data-testid="btn-export" style={{ fontSize:11, fontWeight:500, padding:'5px 12px', border:'1px solid var(--border-m)', background:'transparent', color:'var(--text-2)', borderRadius:4, display:'flex', alignItems:'center', gap:5 }}>
        ...
        <div style={{ border:'1px solid var(--border)', background:'var(--surface)', padding:'22px 24px', borderRadius:6, marginBottom:14 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:18 }}>
            ...
            <div style={{ display:'flex', gap:14, fontSize:11, color:'var(--text-2)' }}>
              <span style={{ display:'flex', alignItems:'center', gap:5 }}><div style={{ width:12, height:3, background:'var(--accent-col)', borderRadius:2 }}/> This year</span>
                ...
                return (
                  <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                    <div style={{ width:'100%', display:'flex', alignItems:'flex-end', gap:2, height:110, justifyContent:'center' }}>
                      ...
                      <div style={{ width:'42%', height:h1, background: recent ? 'var(--accent-col)' : 'var(--blue-col)', borderRadius:'2px 2px 0 0', opacity: recent ? 1 : 0.7 }} />
                    </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 280px', gap:14 }}>
          <div style={{ border:'1px solid var(--border)', background:'var(--surface)', borderRadius:6, overflow:'hidden' }}>
            <div style={{ padding:'16px 20px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              ...
              <button data-testid="btn-view-all-products" style={{ fontSize:11, color:'var(--text-2)', background:'none', border:'1px solid var(--border-m)', padding:'4px 10px', borderRadius:4 }}>View all</button>
            </div>
            <div>
                ...
                <div key={p.rank} style={{ display:'grid', gridTemplateColumns:'28px 1fr 80px 90px 70px 40px', gap:0, padding:'11px 20px', borderBottom:'1px solid var(--border)', alignItems:'center', fontSize:12 }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:5 }}>
                    <div style={{ width:32, height:3, background:'var(--surface-3)', borderRadius:2 }}>
                      <div style={{ width:`${Math.min(p.pct/35*100,100)}%`, height:'100%', background:'var(--accent-col)', borderRadius:2 }} />
                    </div>
                    <span style={{ color:'var(--text-2)', fontSize:11 }}>{p.pct}%</span>
                  </div>
                  <div style={{ display:'flex', justifyContent:'flex-end' }}>
                    {p.trend ? <ArrowUpRight size={13} color="var(--green)"/> : <ArrowDownRight size={13} color="var(--red-col)"/>}
                  </div>
                </div>
              ))}
            </div>
              ...
              {CATEGORIES.map(c=>(
                <div key={c.name}>
                  ...
                  <div style={{ height:5, background:'var(--surface-3)', borderRadius:3 }}>
                    <div style={{ width:`${c.pct}%`, height:'100%', background:'var(--accent-col)', borderRadius:3, opacity: c.pct===72?1:c.pct===18?0.7:c.pct===6?0.45:0.3 }} />
                  </div>
                </div>
              ))}
            </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:3 }}>
                {['M','T','W','T','F','S','S'].map((d,i)=>(
                  <div key={i} style={{ textAlign:'center' }}>
                    ...
                    <div style={{ height:28, borderRadius:3, background: [0.4,0.55,0.6,0.7,1,0.9,0.5][i]===1?'var(--accent-col)':'var(--surface-3)', opacity:[0.4,0.55,0.6,0.7,1,0.9,0.5][i] }} />
                  </div>
                ))}
              </div>
