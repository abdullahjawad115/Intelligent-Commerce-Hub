      ...
      <div style={{ flex:1, overflowY:'auto', padding:'28px 32px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:22 }}>
          ...
          <div style={{ display:'flex', gap:6 }}>
            {['All','Apparel','Accessories','Footwear'].map((c)=>(
              <button key={c} onClick={() => setCategory(c)} data-testid={`filter-cat-${c}`} style={{ fontSize:11, fontWeight:500, padding:'5px 12px', border:'1px solid var(--border-m)', background: category===c?'var(--accent-col)':'transparent', color: category===c?'#0B0A08':'var(--text-2)', borderRadius:4 }}>{c}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'3fr 4fr 4fr', gap:12, marginBottom:16 }}>
          ...
          <div style={{ border:'1px solid var(--red-col)', background:'var(--surface)', borderRadius:6, overflow:'hidden' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', background:'var(--red-dim)', borderBottom:'1px solid var(--red-col)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:7 }}>
                <AlertTriangle size={13} color="var(--red-col)"/>
                <span style={{ fontSize:12, fontWeight:600, color:'var(--red-col)' }}>Critical</span>
              </div>
              <span style={{ fontSize:11, fontWeight:600, padding:'2px 8px', background:'var(--red-dim)', color:'var(--red-col)', border:'1px solid var(--red-col)', borderRadius:3 }}>{CRITICAL.length} SKUs</span>
            </div>
            {CRITICAL.map((item,i)=>(
              <div key={item.sku} style={{ padding:'14px 16px', borderBottom: i<CRITICAL.length-1?'1px solid var(--border)':'none' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, marginBottom:10 }}>
                  ...
                  <div style={{ background:'var(--red-dim)', padding:'7px 10px', borderRadius:4, border:'1px solid var(--red-col)' }}>
                    <div style={{ fontSize:9, color:'var(--red-col)', fontWeight:600, marginBottom:2 }}>DAYS LEFT</div>
                    <div style={{ fontSize:12, fontWeight:700, color:'var(--red-col)' }}>~{item.days}d</div>
                  </div>
                </div>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <span style={{ fontSize:10, color:'var(--text-3)' }}>{item.units} units · {item.exposure}</span>
                  <button data-testid={`btn-critical-rec-${item.sku}`} style={{ fontSize:10, fontWeight:600, padding:'4px 10px', background:'var(--red-col)', color:'#fff', border:'none', borderRadius:3, display:'flex', alignItems:'center', gap:4 }}>
            ...
            {AT_RISK.map((item,i)=>(
              <div key={item.sku} style={{ padding:'12px 16px', borderBottom: i<AT_RISK.length-1?'1px solid var(--border)':'none' }}>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  ...
                  <button data-testid={`btn-at-risk-rec-${item.sku}`} style={{ fontSize:10, fontWeight:600, padding:'4px 10px', background:'transparent', color:'var(--amber)', border:'1px solid var(--amber)', borderRadius:3, display:'flex', alignItems:'center', gap:4 }}>
                    Get rec <ArrowRight size={9}/>
                  </button>
                </div>
              </div>
        ...
        <div style={{ border:'1px solid var(--border)', background:'var(--surface)', borderLeft:'3px solid var(--text-3)', padding:'12px 16px', borderRadius:'0 5px 5px 0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <button data-testid="btn-export-inventory" style={{ fontSize:11, fontWeight:500, padding:'6px 14px', border:'1px solid var(--border-m)', background:'transparent', color:'var(--text-2)', borderRadius:4, whiteSpace:'nowrap', marginLeft:16 }}>Export report</button>
        </div>
