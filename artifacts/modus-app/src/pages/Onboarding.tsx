...
import { Check, ArrowRight, Lock, ChevronRight, ShoppingBag, Globe, Package, Zap } from 'lucide-react';
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', background:'var(--bg)', overflow:'hidden' }}>
      <div style={{ height:52, borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 32px', background:'var(--surface)', flexShrink:0 }}>
        ...
        <nav style={{ display:'flex' }}>
          <div style={{ fontSize:11, fontWeight:500, padding:'0 14px', height:52, display: 'flex', alignItems: 'center', background:'none', borderBottom: '2px solid var(--accent-col)', color: 'var(--text)', letterSpacing:'0.04em', textTransform:'uppercase' }}>ONBOARDING</div>
        </nav>
        <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, color:'var(--text-2)' }}>
          ...
          <div style={{ width:64, height:2, background:'var(--surface-3)', borderRadius:2 }}>
            <div style={{ width:'33%', height:'100%', background:'var(--accent-col)', borderRadius:2 }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'44px 64px' }}>
        <div style={{ maxWidth:860, margin:'0 auto' }}>
          ...
          <div style={{ marginBottom:44 }}>
            <p style={{ fontSize:11, fontWeight:600, color:'var(--accent-col)', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:10 }}>Step 1 — Store Setup</p>
          ...
          <div style={{ marginBottom:36 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
              ...
              <span style={{ fontSize:11, color:'var(--red-col)', fontWeight:500 }}>Required</span>
            </div>
              ...
              {PLATFORMS.map(p => {
                ...
                return (
                  <div key={p.id} onClick={() => !isOn && setConnected(c=>({...c,[p.id]:true}))}
                    ...
                    style={{ border:`1px solid ${isOn ? 'var(--accent-col)' : 'var(--border-m)'}`, background: isOn ? 'var(--accent-dark)' : 'var(--surface)', padding:'18px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', cursor: isOn ? 'default' : 'pointer', transition:'all 0.15s', borderRadius: 6 }}>
                    ...
                    {isOn ? (
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:4, fontSize:11, color:'var(--accent-col)', fontWeight:500, border:'1px solid var(--accent-col)', padding:'5px 12px', borderRadius:4 }}>Connect <ArrowRight size={11} /></div>
                    )}
                  </div>
                );
              })}
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', borderTop:'1px solid var(--border)', paddingTop:24 }}>
            ...
            <Link href="/dashboard">
              <button data-testid="btn-continue" style={{ fontSize:13, fontWeight:600, padding:'11px 28px', background:'var(--accent-col)', color:'#0B0A08', border:'none', borderRadius:5, display:'flex', alignItems:'center', gap:8 }}>
