            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {AI_FEATURES.map(f=>(
                <div key={f.id} style={{ border:'1px solid var(--border)', padding:'13px 14px', display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:10, background:'var(--surface)', borderRadius:5 }}>
                  ...
                  <button data-testid={`toggle-${f.id}`} onClick={()=>setEnabled(s=>({...s,[f.id]:!s[f.id]}))}
                    style={{ flexShrink:0, width:40, height:22, borderRadius:11, background: enabled[f.id] ? 'var(--accent-col)' : 'var(--surface-3)', border: enabled[f.id] ? 'none' : '1px solid var(--border-m)', cursor:'pointer', position:'relative', transition:'background 0.15s', marginTop:2 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:5, marginBottom:16 }}>
              {PLAN.features.map(f=>(
                <div key={f} style={{ display:'flex', alignItems:'center', gap:8, fontSize:12, color:'var(--text-2)' }}>
                  <div style={{ width:4, height:4, borderRadius:'50%', background:'var(--accent-col)' }}/>
                  {f}
                </div>
              ))}
            </div>
