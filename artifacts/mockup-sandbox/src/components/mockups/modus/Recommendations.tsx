...
import { Filter, Zap, ChevronRight, ChevronDown, ChevronUp, ArrowUpRight, BarChart2, TrendingDown, Users, ShoppingCart, Clock } from 'lucide-react';
  ...
  return (
    <div style={{ height:52, borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 32px', background:'var(--surface)', flexShrink:0 }}>
      ...
      <nav style={{ display:'flex' }}>
        {NAV.map(n => (
          <button key={n} style={{ fontSize:11, fontWeight:500, padding:'0 14px', height:52, background:'none', border:'none', borderBottom: n==='Recommendations' ? '2px solid var(--accent)' : '2px solid transparent', color: n==='Recommendations' ? 'var(--text)' : 'var(--text-3)', letterSpacing:'0.04em', textTransform:'uppercase' }}>{n}</button>
        ))}
      </nav>
    ...
    <div style={{ border:'1px solid var(--border-m)', background:'var(--surface)', borderRadius:6, overflow:'hidden', transition:'all 0.2s' }}>
      <div style={{ padding:'18px 22px', display:'grid', gridTemplateColumns:'1fr 200px', gap:20 }}>
        <div>
          ...
          <button onClick={()=>setExpanded(e=>!e)} style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, fontWeight:600, color:'var(--text-3)', background:'none', border:'1px solid var(--border)', padding:'4px 10px', borderRadius:4, transition:'all 0.15s' }}>
        ...
        <div style={{ borderLeft:'1px solid var(--border)', paddingLeft:20, display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
          <div>
            ...
            <div style={{ fontSize:22, fontWeight:700, color: rec.action==='REORDER'?'var(--red)': rec.action==='CLEARANCE'?'var(--text)':'var(--green)', letterSpacing:'-0.5px', marginBottom:6 }}>{rec.impactValue}</div>
            <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, color:'var(--text-2)', fontWeight:500 }}>
              <Zap size={10} color="var(--accent)"/> {rec.confidence}% confidence
            </div>
          </div>
          <button style={{ fontSize:12, fontWeight:600, padding:'9px 0', background:'var(--accent)', color:'#0B0A08', border:'none', borderRadius:4, display:'flex', alignItems:'center', justifyContent:'center', gap:6, marginTop:14 }}>
                  ...
                  <div key={i} style={{ display:'grid', gridTemplateColumns:'24px 1fr 140px 80px', gap:10, alignItems:'center' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                      <div style={{ flex:1, height:4, background:'var(--surface-3)', borderRadius:2 }}>
                        <div style={{ width:`${sig.weight}%`, height:'100%', background:'var(--accent)', borderRadius:2 }}/>
                      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'28px 32px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:22 }}>
          ...
          <div style={{ display:'flex', gap:6, alignItems:'center' }}>
            {(['ALL','BUNDLE','REORDER','DISCOUNT','PROMOTE'] as const).map((f,i)=>(
              <button key={f} style={{ fontSize:11, fontWeight:500, padding:'5px 12px', border:'1px solid var(--border-m)', background: i===0?'var(--accent)':'transparent', color: i===0?'#0B0A08':'var(--text-2)', borderRadius:4 }}>{f}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginTop:18, display:'flex', justifyContent:'space-between', alignItems:'center', borderTop:'1px solid var(--border)', paddingTop:16 }}>
          <span style={{ fontSize:12, color:'var(--text-2)' }}>Showing 5 of 14 recommendations</span>
          <button style={{ fontSize:12, fontWeight:500, padding:'6px 16px', border:'1px solid var(--border-m)', background:'transparent', color:'var(--text-2)', borderRadius:4, display:'flex', alignItems:'center', gap:5 }}>
